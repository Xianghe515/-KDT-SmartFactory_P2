...
function initializeWebSocket() {
    ...
    if (!socket) {
        ...
        console.log('클라이언트가 WebSocket으로 연결 시도...');

        socket.on('connect', function() {
            console.log('클라이언트가 WebSocket으로 성공적으로 연결되었습니다.')
        });
...
function reloadCamera(id) {
    const img = document.getElementById(`camera${id}`);
    img.src = `/camera/stream/${id}?t=${new Date().getTime()}`; // 🔥 캐시 우회
}
...
function updateCameraStatusUI(status) {
    console.log('updateCameraStatusUI 호출됨:', status);
    Object.entries(status).forEach(([cameraId, isConnected]) => {
        const statusEl = document.getElementById(`status-${cameraId}`);
        const connectionStatusEl = document.getElementById(`connection-status-${cameraId}`); // 상태 텍스트 표시 엘리먼트
        const cameraImgEl = document.getElementById(`camera${cameraId}`);

        console.log(`cameraId: ${cameraId}, isConnected: ${isConnected}`);
        console.log(`statusEl:`, statusEl);
        console.log(`connectionStatusEl:`, connectionStatusEl);
        console.log(`cameraImgEl:`, cameraImgEl);

        if (statusEl) {
            statusEl.setAttribute('data-status', isConnected ? 'active' : 'inactivate');
        } else {
            console.warn(`status-${cameraId} 요소를 찾을 수 없습니다.`);
        }

        if (connectionStatusEl) {
            connectionStatusEl.innerText = isConnected ? '연결됨' : '연결 안됨';
        } else {
            console.warn(`connection-status-${cameraId} 요소를 찾을 수 없습니다.`);
        }

        if (isConnected && cameraImgEl) {
            cameraImgEl.src = `/camera/stream/${cameraId}?t=${new Date().getTime()}`;
            console.log(`camera${cameraId} src 업데이트됨: ${cameraImgEl.src}`);
        } else if (!cameraImgEl) {
            console.warn(`camera${cameraId} 요소를 찾을 수 없습니다.`);
        }
    });
}
